...
`calls.createWeb` returns a short-lived LiveKit token. Call it from your backend—never expose an API key in browser code—then pass `token` and `livekitUrl` to your frontend.
